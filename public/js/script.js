...
$(function(){
  ...
  if (search) {
    $.each(search.slice(1).split('&'), (index, param) => {
      ...
      if (index1 > 0) {
        var key = param.slice(0, index1);
        var value = param.slice(index1+1);
  ...
  if (params.searchText && params.searchText.length >= 3) {
    $('[data-search-highlight]').each((index, element) => {
      var $element = $(element);
      var searchHighlight = $element.data('search-highlight');
      var index = params.searchType.indexOf(searchHighlight);

      if (index >= 0) {
        var decodeSearchText = params.searchText.replace(/\+/g, ' ');
        decodeSearchText = decodeURI(decodeSearchText);

        var regex = new RegExp(`(${decodeSearchText})`, 'ig');
...
$(function(){
  function resetTitleEllipsisWidth() {
    $('.board-table .title-text').each(i,e => {
      var $text = $(e);
      var $ellipsis = $(e).closest('.title-ellipsis');
      var $comment = $(e).closest('.titile-container').find('.title-comments');
